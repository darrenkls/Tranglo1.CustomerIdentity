...
namespace Tranglo1.CustomerIdentity.Domain.Entities
{
    public class ComplianceRequisitionType : Enumeration
    {
        ...
        public ComplianceRequisitionType(int id, long complianceSettingTypeCode, string name)
            : base(id, name)
        {
            this.ComplianceSettingTypeCode = complianceSettingTypeCode;
        }

        // IMPORTANT NOTE: Only add compliance requisition type here. Create new requisition type meta if non compliance setting related.
